...
from .ensemble_generator import EnsembleGenerationMixin
from .dropout import replace_dropout
...
class WhiteboxModel(Model):
    ...
    def generate(self, **args):
        ...
        if self.generation_parameters.num_beams > 1:
            beam_ids = torch.clamp(generation["beam_indices"], min=0)
            before_scores = torch.stack(generation["scores"])

            try:
                beam_scores = (
                    before_scores[seq_id, beam_ids[:, seq_id]] for seq_id in range(before_scores.shape[0])
                )
                orig_scores = [s[beam_ids[:, i]].log_softmax(-1) for i, s in enumerate(processor.scores)]
            except Exception as e:
                print(e)
                beam_scores = generation["scores"]
                orig_scores = [s.log_softmax(-1) for s in processor.scores]

            generation.generation_scores = beam_scores
        else:
            orig_scores = [s.log_softmax(-1) for s in processor.scores]
    ...
    def from_pretrained(model_path: str, device: str = "cpu", **kwargs):
        ...
        elif 'mistral' in model_path.lower() or "zephyr" in model_path.lower():
            model_type = "CausalLM"
