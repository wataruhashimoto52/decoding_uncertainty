...
from models.iti_model import ITIWhiteboxModel
from .whitebox_model import WhiteboxModel, Model
from .processor import Processor
from .savers import transfer_dict_list_format, save_list_dict_to_json
from models.sled_model import SLEDWhiteboxModel
from models.fsd_model import FSDWhiteboxModel, FSDVecWhiteboxModel
from models.contrastive_decoding_model import ContrastiveDecodingWhiteboxModel
...
from .register_stat_calculators import register_stat_calculators
...
class UEManager:
    ...
    def __init__(
        ...
        sample_save_file_name: Optional[str] = None,
        est_save_file_name: Optional[str] = None,
        # cal_save_embeddings=0,
        # result_dict_save_file_name=None,
        ass_gt_file_name: Optional[str] = None,
        gpt_gt_file_name: Optional[str] = None,
    ):
        """
        Parameters:
        ...
        * Saving uncertainty estimations, ground-truth uncertainties and ue_metrics values for further usage.
        ...
        for batch_i, (inp_texts, target_texts) in enumerate(iterable_data):
            ...
            for generation_metric in self.generation_metrics:
                if not re.search(r"Rouge|BartScore|PassOne|BLEU|Comet|Align", generation_metric.__class__.__name__):
                    m = generation_metric(
        ...
        for (e_level, e_name), estimator_values in self.estimations.items():
            for (gen_level, gen_name), generation_metric in self.gen_metrics.items():
                for ue_metric in self.ue_metrics:
                    ...
                    if self.model.generation_parameters.num_beams > 1 \
                    and e_name in ("MahalanobisDistanceSeq_decoder", "RDESeq_decoder"):
                        div = len(estimator_values) // len(generation_metric)
                        estimator_values = estimator_values[::div]
