...
class Dataset:
    ...
    SMALL_DATASET_SIZE: Final[int] = 100
    ...
    def from_datasets(
        ...
        **kwargs
        ...
        if "translation" in dataset.column_names:
            x, y = [], []
            source_lang = x_column
            target_lang = y_column
            ...
            for inst in dataset["translation"]:
                ...
                x.append(
                    ...
                    #     text=inst[x_column],
                    # )
                    prompt.format(text=base_dict[source_lang])
                )
                y.append(base_dict[target_lang])
            ...
            for inst in dataset:
                for question, answer in zip(
                    ...
                    y.append(answer)
        elif "strategy-qa" in dataset_name.lower():
            x, y = [], []
            for inst in dataset:
                for question, answer, fact in zip(
                    inst[x_column], inst[y_column], inst["facts"]
                ):
                    x.append(prompt.format(text=question))
                    y.append(" ".join(json.loads(fact) + f" So the answer is {answer}."))
                    
        elif "loogle" in dataset_name.lower():
            x, y = [], []
            for inst in dataset:
                for context, question, answer in zip(
                    inst["context"], inst[x_column], inst[y_column]
                ):
                    x.append(prompt.format(context=context, question=question))
                    y.append(" ".join(json.loads(fact) + f" So the answer is {answer}."))
        ...
        if use_small:
            SMALL_DATASET_SIZE: Final[int] = 100
            if len(x) > SMALL_DATASET_SIZE and len(y) > SMALL_DATASET_SIZE:
                x = x[:SMALL_DATASET_SIZE]
                y = y[:SMALL_DATASET_SIZE]
