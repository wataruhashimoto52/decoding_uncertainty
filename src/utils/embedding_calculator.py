...
class EmbeddingsCalculator(StatCalculator):
    ...
    def __call__(
        ...
        with torch.no_grad():
            generation_parameters = asdict(model.generation_parameters)
            
            if generation_parameters["penalty_alpha"]:
                out = model.generate(
                    **batch,
                    output_scores=False,
                    ...
                    num_beam_groups=generation_parameters["num_beam_groups"],
                    diversity_penalty=generation_parameters["diversity_penalty"],
                )
            else:
                out = model.generate(
                    **batch,
