...
from .scatter_modules import scatter_max, scatter_mean


class HiddenSoftNGram:
    def __init__(self, n, device, vocab_size, sw_coeff=0., stop_words_ids=[], choose="avg"):
        super().__init__()
        assert sw_coeff >= 0.
        self.n = n
    ...
    def penalize(self, prefix, dtype):
        ...
        squeezed_temp_penalty = temp_penalty.squeeze(1)

        
        if self.choose == "avg":
            scatter_mean(squeezed_temp_penalty, prefix[:, self.n-1:], out=penalty)
        if self.choose == "max":
            scatter_max(squeezed_temp_penalty, prefix[:,self.n-1:], out=penalty)
